			  ...
			  const struct loop_config *config)
{
	...
	if (S_ISREG(file->f_mapping->host->i_mode)) {
		rsbac_target = T_FILE;
		rsbac_target_id.dir.device = file->f_path.dentry->d_sb->s_dev;
		rsbac_target_id.dir.inode  = file->f_mapping->host->i_ino;
		rsbac_target_id.dir.dentry_p = file->f_path.dentry;
	}
	else { /* must be block */
		rsbac_target = T_DEV;
		rsbac_target_id.dev.type = D_block;
		rsbac_target_id.dev.major = RSBAC_MAJOR(file->f_mapping->host->i_rdev);
		rsbac_target_id.dev.minor = RSBAC_MINOR(file->f_mapping->host->i_rdev);
	}
